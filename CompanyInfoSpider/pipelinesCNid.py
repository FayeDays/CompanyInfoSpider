...
class CncompanyidSpiderFastPipeline(object):
    ...
    def open_spider(self, spider):
        ...
        # 存入数据之前清空表：
        isexists = self.table_exists(self.cursor, "cn_id")
        if isexists == 1:
            self.cursor.execute("truncate table cn_id")
            self.conn.commit()
        else:
            self.cursor.execute("create table cn_id(id char(6) not null, name char(20) not null, primary key(id))")
            self.conn.commit()
    ...
    def bulk_insert_to_mysql(self, bulkdata):
        try:
            sql = "insert into cn_id (id, name) values(%s, %s)"
    ...
    def process_item(self, item, spider):
        ...
        if len(self.companylist) == 1000:
            print 'len(self.companylist)>>>>>>>>>>>>>>',len(self.companylist)
